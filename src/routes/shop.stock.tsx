...
export const Route = createFileRoute("/shop/stock")({ component: ShopStock });
...
function ShopStock() {
  ...
  const update = useMutation({
    ...
    onSuccess: () => { toast.success("Stock updated"); qc.invalidateQueries({ queryKey: ["shop-products"] }); },
  });
  ...
  return (
    <>
      ...
      <Card className="overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Unit</TableHead><TableHead className="w-32">Stock</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
          <TableBody>
            {(data ?? []).map((p: any) => (
              <TableRow key={p.id}>
